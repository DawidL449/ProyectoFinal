...
public class EspectadoresController {
    ...
    @Autowired
    EspectadoresController EspectadoresRepository;

    @GetMapping("/index")
    public String getAll(Model model) {
        model.addAttribute("categories", EspectadoresRepository.findAll());
    ...
    public String addCategory(@Validated PeliculasModels Peliculas, BindingResult result, Model model) {
        ...
        EspectadoresRepository.save(Peliculas);
        return "redirect:/cat/index";
    }

    @GetMapping("/edit/{id}")
    public String editPeliculas(@PathVariable("id") int id, Model model) {
        PeliculasModels peliculas = (PeliculasModels) EspectadoresController.findById(id).get();
        model.addAttribute("ciudades", EspectadoresRepository);
        return "update";
    }
